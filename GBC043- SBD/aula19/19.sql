...
FROM ATOR
WHERE ATOR.datanasc < '01/01/1960' ;

--4

SELECT nomereal
FROM ATOR
WHERE nomereal LIKE 'S%';

--5

SELECT titulo_original, titulo_pt
FROM FILME
WHERE categoria LIKE '%Romance%';

--6 

SELECT nomereal, nomeartistico
FROM ATOR, FILME, ESTRELA
WHERE FILME.numfilme = ESTRELA.numfilme AND ATOR.cod = ESTRELA.codator AND FILME.titulo_pt = 'Django Livre';

--7

SELECT titulo_original as titulo, categoria
FROM FILME, ATOR, ESTRELA
WHERE FILME.numfilme = ESTRELA.numfilme AND ESTRELA.codator = ATOR.cod AND ATOR.nomeartistico = 'Leonardo DiCaprio';

--8

SELECT nome, titulo_original as titulo, categoria
FROM CLIENTE, EMPRESTIMO, FILME
WHERE CLIENTE.numcliente = EMPRESTIMO.cliente AND EMPRESTIMO.numfilme = FILME.numfilme AND CLIENTE.nome LIKE 'João%';

--9

SELECT titulo_original
FROM EMPRESTIMO,FILME
WHERE EMPRESTIMO.numfilme = FILME.numfilme AND dataret BETWEEN '01/01/2012' AND '31/12/2012';

--10

SELECT titulo_original
FROM EMPRESTIMO, FILME
WHERE EMPRESTIMO.numfilme = FILME.numfilme AND EMPRESTIMO.datedev > '18/10/2016'; 

--11

SELECT nome
FROM CLIENTE, FILME, EMPRESTIMO
WHERE EMPRESTIMO.cliente = CLIENTE.numcliente AND EMPRESTIMO.numfilme = FILME.numfilme AND titulo_pt = 'O Silêncio dos Inocentes';

--12

SELECT nomereal
FROM CLIENTE, FILME, ATOR, EMPRESTIMO, ESTRELA
WHERE CLIENTE.nome = 'Bosco Calcido das Cruzes' AND EMPRESTIMO.numfilme = FILME.numfilme AND ATOR.cod = ESTRELA.codator 
AND ESTRELA.numfilme = FILME.numfilme AND EMPRESTIMO.cliente = CLIENTE.numcliente
ORDER BY nomereal ASC;


--13

SELECT data_lancamento, titulo_original
FROM FILME, CLASSIFICACAO
WHERE FILME.classificacao = CLASSIFICACAO.cod AND CLASSIFICACAO.cod = 1;

--14

CREATE TABLE AtoresConsagrados (
	cod INTEGER NOT NULL,
	datanasc DATE NOT NULL,
	nacionalidade VARCHAR(50) NOT NULL,
	nomereal VARCHAR(50) NOT NULL,
	nomeartistico VARCHAR(50) NOT NULL,
	CONSTRAINT PK_ATORCON PRIMARY KEY (cod),
	CONSTRAINT FK_ATORCON FOREIGN KEY (cod)
		REFERENCES ATOR
);

INSERT INTO AtoresConsagrados
SELECT *
FROM ATOR
WHERE ATOR.datanasc < '01/01/1970';

SELECT *
FROM AtoresConsagrados;
