...
int** matriz_C_aleatorio(int **M, int n, int Iniciol, int Inicioc, int Linha1, int Coluna1, int *P)
{
    int i, j, li, co, tam, p =((n*n)/5) ;
    int k, ra, c, c2, MM, loop = 0;
    srand((unsigned)time(NULL));
    ...
    for(j = 0 ; j < p ; j++, loop++)
    {
        ...
        do
        {
            li = rand()%n;
            co = rand()%n;
            if(M[li][co] == 0 && li != Iniciol && co != Inicioc && li != Linha1 && co != Coluna1) break;
        }
        while(1);
        tam = (rand()%p)+1;
        //printf("\n Linha : %d || Coluna : %d || Tamanho : %d",li,co,tam);
        //printf(" VALOR : %d", M[li][co]);
        for( ; tam > 0 ; tam--)
        {
            c = 0;
            c2 = 0;
            MM = 0;
            P = matriz_possiveis(M,n,li,co,P);
            if(P[0] == -1)
            {
                printf("\nError em Posicao");
                break;
...
int matriz_percorreH(int **M, int n, int *Linha, int *Coluna, int *P, int H)
{
    int auxp, c = 0, MM = 0, i;
    ...
    if(H == 0) M[*Linha][*Coluna] = 4;
    if(H == 4) M[*Linha][*Coluna] = 5;

    if(auxp == 1 || auxp == 2 || auxp == 12) P[0] = 0;
    else
    {
        if(M[(*Linha-1)][*Coluna] == H) P[0] = 1;
    }
    if(auxp == 2 || auxp == 4 || auxp == 24) P[1] = 0;
    else
    {
        if(M[*Linha][(*Coluna+1)] == H) P[1] = 1;
    }
    if(auxp == 3 || auxp == 4 || auxp == 34) P[2] = 0;
    else
    {
        if(M[(*Linha+1)][*Coluna] == H) P[2] = 1;
    }
    if(auxp == 1 || auxp == 3 || auxp == 13) P[3] = 0;
    else
    {
        if(M[*Linha][(*Coluna-1)] == H) P[3] = 1;
    }
...
int matriz_voltaH(int **M, int n, int Linha, int Coluna, int H)
{
    int cont=0;
    int aux = matriz_canto(n,Linha,Coluna);
    if(aux == 1)
    ...
    if(aux == 12)
    {
        if(M[0][(Coluna+1)] == H) cont++;
        if(M[0][(Coluna-1)] == H) cont++;
        if(M[1][Coluna] == H) cont++;
        return cont;
    }
    if(aux == 34)
    {
        if(M[(n-1)][(Coluna+1)] == H) cont++;
        if(M[(n-1)][(Coluna-1)] == H) cont++;
        if(M[(n-2)][Coluna] == H) cont++;
        return cont;
    }
    if(aux == 13)
    {
        if(M[(Linha+1)][0] == H) cont++;
        if(M[(Linha-1)][0] == H) cont++;
        if(M[Linha][1] == H) cont++;
        return cont;
    }
    if(aux == 24)
    {
        if(M[(Linha+1)][(n-1)] == H) cont++;
        if(M[(Linha-1)][(n-1)] == H) cont++;
        if(M[Linha][(n-2)] == H) cont++;
        return cont;
    }
    if(aux == 0)
    {
        if(M[Linha][(Coluna+1)] == H) cont++;
        if(M[Linha][(Coluna-1)] == H) cont++;
        if(M[(Linha+1)][Coluna] == H) cont++;
        if(M[(Linha-1)][Coluna] == H) cont++;
        return cont;
    }
    if(aux == 5) return (7);
...
int* matriz_possiveis2(int **M, int n, int Linha, int Coluna, int *P)
{
    int auxp;
    P[0] = 0;
    P[1] = 0;
    ...
    {
        if(M[(Linha-1)][Coluna] == 0) P[0] = 1;
    }
    if(auxp == 2 || auxp == 4 || auxp == 24) P[1] = 0;
    else
    {
        if(M[Linha][(Coluna+1)] == 0) P[1] = 1;
    }
    if(auxp == 3 || auxp == 4 || auxp == 34) P[2] = 0;
    else
    {
        if(M[(Linha+1)][Coluna] == 0) P[2] = 1;
    }
    if(auxp == 1 || auxp == 3 || auxp == 13) P[3] = 0;
    else
    {
        if(M[Linha][(Coluna-1)] == 0) P[3] = 1;
    }
    return P;
}
